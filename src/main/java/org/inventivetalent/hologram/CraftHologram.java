...
abstract class CraftHologram implements Hologram {
	...
	protected void buildPackets(boolean rebuild) throws Exception {
		...
		if (HologramAPI.is1_8 && !HologramAPI.useProtocolSupport) {
			Object armorStand = ClassBuilder.buildEntityArmorStand(world, getLocation().add(0.0D, -1.25D, 0.0D), getText());
			...
			this.dataWatcherArmorStand = AccessUtil .setAccessible(PacketPlayOutSpawnEntityLivingFieldResolver.resolveByFirstType(NMSClass.DataWatcher)).get(spawnPacketArmorStand);

			this.teleportPacketArmorStand = ClassBuilder.buildTeleportPacket(hologramIDs[0], getLocation().add(0.0D, -1.25D, 0.0D), true, false);
		} else {
			Object horse_1_7 = ClassBuilder.buildEntityHorse_1_7(world, getLocation().add(0.0D, 54.560000000000002D, 0.0D), getText());
			Object horse_1_8 = ClassBuilder.buildEntityHorse_1_8(world, getLocation().add(0.0D, -2.25D, 0.0D), getText());

			Object witherSkull_1_7 = ClassBuilder.buildEntityWitherSkull(world, getLocation().add(0.0D, 54.560000000000002D, 0.0D));
			...
			this.dataWatcherHorse_1_7 = AccessUtil.setAccessible(PacketPlayOutSpawnEntityLivingFieldResolver.resolveByFirstType(NMSClass.DataWatcher)).get(spawnPacketHorse_1_7);

			this.spawnPacketHorse_1_8 = ClassBuilder.buildHorseSpawnPacket_1_8(horse_1_8, getText());
			this.dataWatcherHorse_1_8 = AccessUtil.setAccessible(PacketPlayOutSpawnEntityLivingFieldResolver.resolveByFirstType(NMSClass.DataWatcher)).get(spawnPacketHorse_1_8);
			...
			if (Minecraft.VERSION.olderThan(Minecraft.Version.v1_9_R1)) {
				...
				AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("a")).set(attachPacket, hologramIDs[1]);
				AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b")).set(attachPacket, hologramIDs[0]);
			}
			if (!HologramAPI.is1_8 || HologramAPI.useProtocolSupport) {
				this.teleportPacketSkull = ClassBuilder.buildTeleportPacket(hologramIDs[0],getLocation().add(0.0D, 54.560000000000002D, 0.0D), true, false);
		...
		if (Minecraft.VERSION.olderThan(Minecraft.Version.v1_9_R1)) {
			...
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("a")).set(ridingAttachPacket, 0);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("a")).set(ridingEjectPacket, 0);

			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b")).set(ridingAttachPacket, hologramIDs[0]);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("b")).set(ridingEjectPacket, hologramIDs[0]);

			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("c")).set(ridingAttachPacket, getAttachedTo() != null ? getAttachedTo().getEntityId() : -1);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutAttachEntity.getDeclaredField("c")).set(ridingEjectPacket, -1);
		} else {
			...
			AccessUtil.setAccessible(NMSClass.PacketPlayOutMount.getDeclaredField("a")).set(ridingAttachPacket, (((DefaultHologram) this).isAttached()) && (getAttachedTo() != null) ? getAttachedTo().getEntityId(): -1);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutMount.getDeclaredField("a")).set(ridingEjectPacket, (((DefaultHologram) this).isAttached()) && (getAttachedTo() != null) ? getAttachedTo().getEntityId() : -1);

			AccessUtil.setAccessible(NMSClass.PacketPlayOutMount.getDeclaredField("b")).set(ridingAttachPacket, hologramIDs[0]);
			AccessUtil.setAccessible(NMSClass.PacketPlayOutMount.getDeclaredField("b")).set(ridingEjectPacket, new int[0]);
		}
		if (!rebuild) {
	...
	protected void sendNamePackets(Collection<? extends Player> receivers) {
		for (Player p : receivers) {
			try {
				int id = HologramAPI.is1_8 && !HologramAPI.useProtocolSupport ? hologramIDs[0] : HologramAPI.getVersion(p) > 5 ? hologramIDs[2] : hologramIDs[1];
				Object dataWatcher = HologramAPI.is1_8 && !HologramAPI.useProtocolSupport ? dataWatcherArmorStand : HologramAPI.getVersion(p) > 5 ? dataWatcherHorse_1_8 : dataWatcherHorse_1_7;
				...
				if ((HologramAPI.getVersion(p) <= 5) && (hologramIDs.length > 1)) {
					HologramAPI.sendPacket(p, ClassBuilder.buildNameMetadataPacket(hologramIDs[1], dataWatcherHorse_1_7, 10, 11, getText()));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public String toString() {
		return "{\"hologramIDs\":\"" + Arrays.toString(hologramIDs) + "\",\"touchIDs\":\"" + Arrays.toString(touchIDs)
				+ "\",\"packetsBuilt\":\"" + packetsBuilt + "\",\"spawnPacketArmorStand\":\""
				+ spawnPacketArmorStand + "\",\"spawnPacketWitherSkull\":\"" + spawnPacketWitherSkull
				+ "\",\"spawnPacketHorse_1_7\":\"" + spawnPacketHorse_1_7 + "\",\"spawnPacketHorse_1_8\":\""
				+ spawnPacketHorse_1_8 + "\",\"attachPacket\":\"" + attachPacket
				+ "\",\"teleportPacketArmorStand\":\"" + teleportPacketArmorStand + "\",\"teleportPacketSkull\":\""
				+ teleportPacketSkull + "\",\"teleportPacketHorse_1_7\":\"" + teleportPacketHorse_1_7
				+ "\",\"teleportPacketHorse_1_8\":\"" + teleportPacketHorse_1_8 + "\",\"destroyPacket\":\""
				+ destroyPacket + "\",\"spawnPacketTouchSlime\":\"" + spawnPacketTouchSlime
				+ "\",\"spawnPacketTouchWitherSkull\":\"" + spawnPacketTouchVehicle + "\",\"attachPacketTouch\":\""
				+ attachPacketTouch + "\",\"destroyPacketTouch\":\"" + destroyPacketTouch
				+ "\",\"teleportPacketTouchSlime\":\"" + teleportPacketTouchSlime
				+ "\",\"teleportPacketTouchWitherSkull\":\"" + teleportPacketTouchVehicle
